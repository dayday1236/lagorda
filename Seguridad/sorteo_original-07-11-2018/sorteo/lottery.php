...
$host = "185.91.108.50";
$username = "sorteo@lagordadenavidad.es";
$password = "R0ueq*69";
		...
		if (!$existe) {
			...
			if (!$existe) {
				...
				if($premios == ($num_premios_dia-1) && $winner == 1){				
                                        ...
                                        $exito = enviar_sec_mail_html($host, $username, $password, $fromname, $host, $to, $asunto, 'Se han repartido los '.$num_premios_dia.' premios de hoy de '.$cliente.'');
				}
                                
                                if($ganadores_totales == $max_premios && $winner == 1){	
                                        $winner = 0;
                                        $ato = array('camino@sitelicon.com','mufarte@gmail.com','fernando.sitelicon@gmail.com');
                                        $exito = enviar_sec_mail_html($host, $username, $password, $fromname, $host, $to, $asunto, 'Se han repartido los '.$max_premios.' premios totales de '.$cliente.'');
				}
			}
			if ($status_landing != 1) {
                            $winner = 0;                            
			...
			$winner = 0;  // El email existe, no es ganador, y no se guarda en la bbdd
		}
	} else {
		$winner = 0; // No ha introducido mail
	}
}else{
	if ($_POST["email"] != "") {
//		echo 'else';
	...
	$html = '	<html>
				...
				<body>
					<table style="width:100%;background-color:#fff;">
						<tr>
                                                    <td style="text-align:center"><a href="https://docs.google.com/forms/d/e/1FAIpQLSe7BvuomGeu9ccRkIswN0r3CvWbAB1U1vcaex01mM5EXNQzeQ/viewform?usp=sf_link"><img src="'.$url_tienda.'/sorteo/img/email-ganadores-copy.png"></img></a></td>
						</tr>
					</table>
				</body>
			</html>';
						...
						<tr>
							<td style="text-align:center">
								<a href="http://www.google.com/url?q=http%3A%2F%2Flagordadenavidad.es%2Fcomprar%2Fbilletes%3Futm_source%3Dnewsletter%26utm_medium%3Demail%26utm_campaign%3Demail_nowin&sa=D&sntz=1&usg=AFQjCNHxh2FIOdGcA1PZxxg3YlBCyAPnDw">                                                                
									<img src="'.$url_tienda.'/sorteo/img/email-no-ganadores-copy-2.png"/>
								</a>
							</td>
						</tr>
					</table>
...
$exito = enviar_sec_mail_html($host, $username, $password, $fromname, $username, $to, $asunto, $html);
